...
namespace OutlookGoogleCalendarSync {
    ...
    public class GoogleCalendar {
        ...
        private Event createCalendarEntry(AppointmentItem ai) {
            ...
            AddOutlookID(ref ev, ai,hostname);
        ...
        #endregion
        public static Boolean hasLocalOutlookId(Event ev, String entryId, String machineId)
        {
            String outlookid = "";
            return hasLocalOutlookId(ev, entryId, machineId, out outlookid);
        }
        public static Boolean hasLocalOutlookId(Event ev, String entryId, String machineId, out String outlookId)
        {
            String Ids;
            if (GetOGCSproperty(ev, entryId, out Ids))
            {
                String[] Pairs = Ids.Split(';');
                foreach(String pair in Pairs)
                {
                    if(pair.StartsWith(machineId.ToLower()+":"))
                    {
                        outlookId = pair.Split(':')[1];
                        return true;
                    }
                }
            }
        ...
        public void ReclaimOrphanCalendarEntries(ref List<Event> gEvents, ref List<AppointmentItem> oAppointments, Boolean neverDelete = false) {
            ...
            for (int g = gEvents.Count-1; g >= 0; g--) {
                ...
                if (!hasLocalOutlookId(ev, oEntryID,hostname)) {
        ...
        public void IdentifyEventDifferences(
            ...
            for (int g = google.Count - 1; g >= 0; g--) {
                if (hasLocalOutlookId(google[g], oEntryID,hostname, out compare_gEntryID)) {
                    for (int o = outlook.Count - 1; o >= 0; o--) {
                        ...
                        if (compare_gEntryID.StartsWith(outlook[o].EntryID)) {
                            ...
                            Event ev = google[g];
                            AddOutlookID(ref ev, outlook[o],hostname);
                            UpdateCalendarEntry_save(ev);
                            google[g] = ev;
        ...
        public static void AddOutlookID(ref Event ev, AppointmentItem ai,String machineId) {
            //Add the Outlook appointment ID into Google event.
            //This will make comparison more efficient and set the scene for 2-way sync.
            //ensure we dont kill other outlookid's for other sync users...
            //format is hostname:outlookid;hostname:outlookid;...
            String val = "";
            String oId = OutlookCalendar.Instance.IOutlook.GetGlobalApptID(ai);
            bool found = false;
            if (GetOGCSproperty(ev, oEntryID, out val))
            { 
                ...
                {
                    if (!pairs[x].Contains(":")) continue; //remove old style ids...
                    if(pairs[x].StartsWith(machineId.ToLower()+":"))
                    {
                        pairs[x] = machineId.ToLower() + ":" + oId + ";";
                        found = true;
                    }
                    val += pairs[x]+";";
                }
            ...
            if(!found)
            {
                val += machineId.ToLower() + ":" + oId + ";";
            }
            addOGCSproperty(ref ev, oEntryID, val);
        }
